...
def get_resnet(hparams=None):

    resnets = {
        "resnet18": ResNet18(cifar_head=(hparams.data == 'cifar')),
        "resnet50": ResNet50(cifar_head=(hparams.data == 'cifar'), hparams=hparams)
    }

    if hparams.arch not in resnets.keys():
        raise KeyError(f"{hparams.arch} is not a valid ResNet version")

    return resnets[hparams.arch]