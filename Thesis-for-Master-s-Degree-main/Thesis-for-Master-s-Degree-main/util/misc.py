...
def init_distributed_mode(cfg):
    ...
    torch.cuda.set_device(config.DDP.GPU)
    cfg.DIST_BACKEND = 'nccl'
    print('| distributed init (rank {}): {}, gpu {}'.format(cfg.RANK, cfg.DIST_URL, cfg.GPU), flush=True)
...
def save_model(cfg, epoch, model, model_without_ddp, optimizer, loss_scaler):
    ...
    if loss_scaler is not None:
        ...
        model.save_checkpoint(save_dir=config.FINETUNE.OUTPUT_DIR, tag="checkpoint-%s" % epoch_name, client_state=client_state)
...
def load_model(config, model_without_ddp, optimizer, loss_scaler):
    if config.RESUME:
        ...
        if 'optimizer' in checkpoint and 'epoch' in checkpoint and not (hasattr(config, 'eval') and config.EVAL):
            optimizer.load_state_dict(checkpoint['optimizer'])
            config.defrost()
            config.START_EPOCHS = checkpoint['epoch'] + 1
            config.freeze()
